...
def worker(job_num, worker_num, good_grasp):
    ...
    object_name = os.path.basename(file_list[job_num])
    data_dir = os.path.join(file_dir, object_name, 'google_512k')  # Note
    ...
    cloud = o3d.io.read_point_cloud(data_dir + "/nontextured.ply")  # Note
    ...
    while np.sum(good_count_perfect < grasp_num_per_fc) != 0:
        ...
        grasps = ags.sample_grasps(obj, num_grasps=target_num_grasps, max_num_samples=max_num_samples, minx=None)
...
if __name__ == '__main__':
    file_dir = "../Dataset/ycb/ycb_meshes_google"  # Note
