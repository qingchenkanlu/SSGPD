...
//                      << " (squared distance: " << pointRadiusSquaredDistance[0] << ")" << std::endl;
    ...
    writer.writeASCII("../surface_cloud.pcd", *cloud_plane_filter); // 保存桌面以上，计算过法线的点云
    writer.writeASCII("../surface_cloud_with_normals.pcd", *surface_cloud_with_normal); // 保存桌面以上，计算过法线的点云
    writer.writeASCII("../surface_cloud_voxel.pcd", *cloud_workspace_voxel); // 保存未滤除桌面，降采样之后的点云
