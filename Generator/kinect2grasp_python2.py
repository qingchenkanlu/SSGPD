...
# global config:
yaml_config = YamlConfig(os.environ['HOME'] + "/code/grasp-pointnet/dex-net/test/sample_config.yaml")
...
def check_collision_square(grasp_bottom_center, approach_normal, binormal,
    ...
    if not use_dataset_py:
        ...
        if len(points_in_area) == 0:
            ...
            has_p = True
    # for the way of pointGPD/dataset_ycb_old.py:
    else:
...
if __name__ == '__main__':
    ...
    while not rospy.is_shutdown():
        ...
        if kinect_data.data == '':
            ...
            continue
        real_grasp, points, normals_cal = cal_grasp(kinect_data, cam_pos)  # NOTE:获取抓取姿态
        ...
        in_ind, in_ind_points = collect_pc(real_grasp, points)  # NOTE:计算手抓坐标下的点云坐标
        ...
        for ii in range(len(in_ind_points)):
            ...
            if in_ind_points[ii].shape[0] < minimal_points_send_to_point_net:  # 点数太少
                ...
                for _ in range(repeat):
                    ...
                    if_good_grasp, grasp_score_tmp = test_network(model.eval(), points_modify)  # NOTE:调用模型预测得分
