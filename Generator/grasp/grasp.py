...
class PointGrasp(Grasp):
    ...
    @abstractmethod
    def create_line_of_action(start_point, axis, max_width, num_samples):
...
class ParallelJawPtGrasp3D(PointGrasp):
    ...
    @property
    def open_width(self):
        """ float : maximum opening max_width of the jaws """
        return self.max_grasp_width_

    @property
    def close_width(self):
        """ float : minimum opening max_width of the jaws """
    ...
    def endpoints(self):
        ...
        -------
        :obj:`numpy.ndarray`
            location of jaws(fingers) in 3D space at max opening max_width
    ...
    def configuration_from_params(center, axis, max_width, angle=0, depth=0, min_width=0, normal=[], minor_pc=[]):
    ...
    def params_from_configuration(configuration, type='axis'):
        ...
        min_width : float
            minimum closing max_width of jaws
    ...
    def width_from_endpoints(g1, g2):
        """ max_width of grasp from endpoints """
        grasp_axis = g2 - g1
        return np.linalg.norm(grasp_axis)

    @staticmethod
    def grasp_from_endpoints(g1, g2, max_width=None, approach_angle=0, close_width=0):
        ...
        max_width : float
            maximum opening max_width of jaws
        approach_angle : float
            approach angle of grasp
        close_width : float
            max_width of gripper when fully closed
        """
        x = ParallelJawPtGrasp3D.center_from_endpoints(g1, g2)
        v = ParallelJawPtGrasp3D.axis_from_endpoints(g1, g2)
        if max_width is None:
            max_width = ParallelJawPtGrasp3D.width_from_endpoints(g1, g2)
        return ParallelJawPtGrasp3D(
            ParallelJawPtGrasp3D.configuration_from_params(x, v, max_width, min_width=close_width, angle=approach_angle))
    ...
    def create_line_of_action(start_point, axis, max_width, num_samples):
        ...
        line_of_action = [start_point + t * axis for t in np.linspace(0, float(max_width), num=num_samples)]
