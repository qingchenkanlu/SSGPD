...
def main(args):
    ...
    file_dir = Path(str(experiment_dir) + '/%sSSGPD-' % args.model_name + str(datetime.datetime.now().strftime('%Y-%m-%d_%H-%M')))
    ...
    """DATA LOADING"""
    trainDataset = OneViewDatasetLoader(grasp_points_num=1024, dataset_path=datapath, tag='train')
    testDataset = OneViewDatasetLoader(grasp_points_num=1024, dataset_path=datapath, tag='test')
    ...
    for epoch in range(start_epoch,args.epoch):
        ...
        for batch_id, data in tqdm(enumerate(trainDataLoader, 0), total=len(trainDataLoader), smoothing=0.9):
            ...
            points, target = points.float().cuda(), target.long().squeeze().cuda()  # Mark
            optimizer.zero_grad()
