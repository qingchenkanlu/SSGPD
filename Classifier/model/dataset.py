...
class PointGraspOneViewDataset(torch.utils.data.Dataset):
    ...
    def collect_pc(self, grasp, pc, transform, label):
        """
        获取手抓闭合区域中的点云
        ...
        pc_c2b = pc_m2c[self.in_ind] + [ags.gripper.hand_depth, 0, 0]  # 中心点坐标系下闭合区域点云转换到手抓底部坐标系下
        ...
        if vis:  # 显示手抓闭合区域内点云
            ...
            show_points(pc, color='b', scale_factor=.002)  # 原始点云
            show_points(pc_p2m, color='b', scale_factor=.002)  # 配准到扫描仪坐标系下点云
        ...
        if vis:  # 显示手抓闭合区域内点云
            ...
            #                                          "ycb_meshes_google/003_cracker_box/google_512k/nontextured.ply"))
            # 世界坐标系
            # show_line([0, 0, 0], [0.07, 0, 0], color='r', scale_factor=.0020)
            # show_line([0, 0, 0], [0, -0.07, 0], color='g', scale_factor=.0020)
            # show_line([0, 0, 0], [0, 0, 0.07], color='b', scale_factor=.0020)
            mlab.quiver3d(0, 0, 0, 1, 0, 0, scale_factor=.08, line_width=0.2, color=(1, 0, 0), mode='arrow')  # x
            mlab.quiver3d(0, 0, 0, 0, -1, 0, scale_factor=.08, line_width=0.2, color=(0, 1, 0), mode='arrow')  # y
            mlab.quiver3d(0, 0, 0, 0, 0, 1, scale_factor=.08, line_width=0.2, color=(0, 0, 1), mode='arrow')  # z
            #
            # # show_points(pc, color='b', scale_factor=.002)  # 原始点云
            ...
            # 中心点坐标系下手抓(应在世界坐标系原点)
            hand_points = (np.dot(matrix_m2c, (hand_points - center).T)).T  # 手抓关键点转换到中心点坐标系
            ags.show_grasp_3d(hand_points, color='d')  # 显示手抓
            ...
            # triangles = [(0, 1, 2), (0, 2, 3), (4, 5, 6), (4, 6, 7), (1, 5, 6), (1, 2, 6),
            #              (0, 4, 7), (0, 3, 7), (2, 3, 6), (3, 6, 7), (0, 1, 5), (0, 4, 5)]
            # mlab.triangular_mesh(x, y, z, triangles, color=(0, 0, 1), opacity=0.5)
        ...
        if vis:
            ...
            grasp_bottom_center = -ags.gripper.hand_depth * approach + center
            hand_points = ags.get_hand_points(grasp_bottom_center, approach, binormal)
            ags.show_grasp_3d(hand_points, color=(1.0, 0.0, 0.0))

            pc_c2m_region = (np.dot(matrix_m2c.T, pc_m2c[self.in_ind].T)).T + center  # 扫描仪坐标系下手抓闭合区域中的点云
            show_points(pc_c2m_region, color='g', scale_factor=.002)
            ...
            # 中心点坐标系下手抓(应在世界坐标系原点)
            hand_points = (np.dot(matrix_m2c, (hand_points - center).T)).T  # 手抓关键点转换到中心点坐标系
            hand_points = hand_points + [ags.gripper.hand_depth, 0, 0]  # 手抓关键点转换到手抓底部坐标系
            ags.show_grasp_3d(hand_points, color='d')  # 显示手抓

            show_points(pc_c2b, color='b', scale_factor=.002)  # 中心点坐标系下手抓闭合区域中的点云
            ...
            triangles = [(0, 1, 2), (0, 2, 3), (4, 5, 6), (4, 6, 7), (1, 5, 6), (1, 2, 6),
                         (0, 4, 7), (0, 3, 7), (2, 3, 6), (3, 6, 7), (0, 1, 5), (0, 4, 5)]
            mlab.triangular_mesh(x, y, z, triangles, color=(1, 1, 0), opacity=0.2)
        ...
        if self.projection:
            ...
            return pc_c2b  # 返回手抓闭合区域中的点云(手抓底部坐标系下)
    ...
    def __getitem__(self, index):
        ...
        grasp_pc = self.collect_pc(grasp, pc, t, label)  # NOTE:获取手抓闭合区域中的点云
